import constantes as c
import fonctions_plot as fp
import matplotlib.pyplot as plt
import numpy as np

if __name__ == "__main__":
    # Tracer la distance entre la Terre et Vénus
    fp.plot_distance_terre_venus()
    fp.plot_densite_elec()
    fp.plot_densite_elec_SEP_precis()
    fp.repre_dens_soleil()
    fp.plot_sigma_log_amplitude_2D()
    fp.plot_I1()
    fp.plot_I2()
    fp.plot_I3()
    fp.plot_angular_variance()
    fp.plot_RMS()
    fp.plot_spectre_phase()
    fp.plot_spectre_doppler()
    fp.plot_serie_temporelle_doppler()
    fp.plot_serie_temp_2()
    fp.plot_reconstruction_phase_deg()
    fp.plot_reconstruction_phase_deg_diff_SEP()
    fp.plot_bilan_liaison()